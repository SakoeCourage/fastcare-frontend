  ...
  return <div className='flex bg-gray-50/60 p-2 rounded-md border items-center gap-2 w-full relative'>
    ...
    <nav className=' text-xs absolute right-0 p-2 bg-gray-100 inset-y-0 flex  items-center justify-center'>
      <Moretableoptions options={[{
        ...
        theme: "primary",
        onOptionSelected: ()=>setShowNewFamilyMemberForm(true)
      },
      {
        optionName: "Remove",
      ...
      {/* <button onClick={()=>setShowNewFamilyMemberForm(true)} className='flex grow border-b border-b-gray-200 items-center text-green-400 p-1 justify-start gap-1'>
        <IconifyIcon fontSize="1rem" icon='bitcoin-icons:edit-outline' className=' bg-transparent !h-5 !w-5 !p-0' />
        <span>Edit</span>
      </button>
      <button onClick={()=>setShowNewFamilyMemberForm(true)} className='flex grow items-center text-red-400 p-1 justify-start gap-1'>
